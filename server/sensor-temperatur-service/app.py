...
from models import TempSensorTable 
...
def mqtt_messaging(reading_):
    if reading_.reading > reading_.setpoint + reading_.margin:
        ...
        mqtt_client.publish(f"greenhouse_app/{reading_.greenhouse_id}/heaters", '{"command":"OFF"}', retain = True) 
        ...
        mqtt_client.publish(f"greenhouse_app/{reading_.greenhouse_id}/heaters", '{"command":"ON"}', retain = True)
...
def get_all_readings(greenhouse_id):
    try:
        all_readings = TempSensorTable.query.filter(greenhouse_id = greenhouse_id).all()
...
def get_latest_reading(greenhouse_id):
    try:
        latest_reading = TempSensorTable.query.filter(greenhouse_id = greenhouse_id).order_by(TempSensorTable.timestamp.desc()).first()
