...
from models import FansActuator 
...
def get_all_readings(greenhouse_id):
    try:
        all_statuses = FansActuator.query.filter(greenhouse_id = greenhouse_id).all()
...
def get_latest_reading(greenhouse_id):
    try:
        latest_status = FansActuator.query.filter(greenhouse_id = greenhouse_id).order_by(FansActuator.timestamp.desc()).first()
