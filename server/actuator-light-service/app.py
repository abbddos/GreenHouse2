...
from models import LightsActuator 
...
def get_all_readings(greenhouse_id):
    try:
        all_statuses = LightsActuator.query.filter(greenhouse_id = greenhouse_id).all()
...
def get_latest_reading(greenhouse_id):
    try:
        latest_status = LightsActuator.query.filter(greenhouse_id = greenhouse_id).order_by(LightsActuator.timestamp.desc()).first()
