...
from models import VentsActuator 
...
def get_all_readings(greenhouse_id):
    try:
        all_statuses = VentsActuator.query.filter(greenhouse_id = greenhouse_id).all()
...
def get_latest_reading(greenhouse_id):
    try:
        latest_status = VentsActuator.query.filter(greenhouse_id = greenhouse_id).order_by(VentsActuator.timestamp.desc()).first()
