...
from models import CurtainsActuator 
...
def get_all_readings(greenhouse_id):
    try:
        all_statuses = CurtainsActuator.query.filter(greenhouse_id = greenhouse_id).all()
...
def get_latest_reading(greenhouse_id):
    try:
        latest_status = CurtainsActuator.query.filter(greenhouse_id = greenhouse_id).order_by(CurtainsActuator.timestamp.desc()).first()
