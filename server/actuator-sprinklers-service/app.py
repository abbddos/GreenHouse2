...
from models import SprinklersActuator 
...
def get_all_readings(greenhouse_id):
    try:
        all_statuses = SprinklersActuator.query.filter(greenhouse_id = greenhouse_id).all()
...
def get_latest_reading(greenhouse_id):
    try:
        latest_status = SprinklersActuator.query.filter(greenhouse_id = greenhouse_id).order_by(SprinklersActuator.timestamp.desc()).first()
